...
1. `artist_albums()` + `album_tracks()` - Get tracks from seed artist's albums
...
**Strategy weights** (default): genre 35, throwback 30, artist_search 25, artist_albums 5, album 5. Bender search strategies paginate (2 pages of 10) since Spotify's search limit max is now 10.
...
**Spotify API tracking** (12 event types): `spotify_api_search`, `spotify_api_track`, `spotify_api_artist`, `spotify_api_artist_albums`, `spotify_api_album_tracks`, `spotify_api_get_track`, `spotify_api_get_episode`, `spotify_api_devices`, `spotify_api_transfer`, `spotify_api_status`, `spotify_api_rate_limited`, `spotify_api_error`. Instrumented at every Spotify API call site in `db.py` and `app.py`.
